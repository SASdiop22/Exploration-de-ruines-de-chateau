...
bool terrain::estCaseAccessible(int ligne, int colonne) const
{    aventurier A{};
    if (ligne >= 0 && ligne < hauteur() && colonne >= 0 && colonne < largeur()) {
        // Vérifier si la case est vide (' ') ou il y'a l'aventurier dans la case
        return (d_tableau[ligne][colonne] == ' ' || d_tableau[ligne][colonne]==  A.symbole());
    }
